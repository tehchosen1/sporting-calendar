...
export const fetchMatchesForMonthYear = async (
  ...
  const localStorageData = typeof window !== "undefined" ? localStorage.getItem(key) : null;
  ...
  if (localStorageData) {
    ...
    if (month !== currentMonth || year !== currentYear || cachedData.length === 0) {
...
async function fetchDataFromServer(month: number, year: number, key: string): Promise<MatchInfo[]> {
  try {
    const response = await axios.get(TEAM_URL, { headers: { "X-Requested-With": "XMLHttpRequest" } });
    ...
    $("#team_games .stats tr.parent").each((_, element) => {
      ...
      if (date.getMonth() + 1 === month && date.getFullYear() === year) {
        const matchInfo: MatchInfo = {
          ...
          teamIcon: BASE_NOPROXY_URL + ($(element).find("td a img").attr("src")?.trim() || ""),
          teamName: $(element).find("td.text a").eq(0).text().trim(),
          leagueIcon: BASE_URL + ($(element).find("td img").eq(1).attr("src")?.trim() || ""),
          leagueName: $(element).find("td .micrologo_and_text .text a").eq(0).text().trim(),
...
export const scrapePlayerImage = async () => {
  try {
    ...
    const playerPageResponse = await axios.get(SPORTING_BASE_URL + playerPageUrl, { headers: { "X-Requested-With": "XMLHttpRequest" } });
